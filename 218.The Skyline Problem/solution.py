...
write a program to output the skyline formed by these buildings
collectively (Figure B).

 Buildings  Skyline Contour
The geometric information of each building is represented by a triplet
of integers [Li, Ri, Hi], where Li and Ri are the x coordinates of the left
...
and always has zero height. Also, the ground in between any two
...
class Solution(object):
    def getSkyline(self, buildings):
        ...
        """
        skyline = []
        # hr[0] is height, hr[1] is right. Forming a max heapq
        hr = []
        i, n = 0, len(buildings)
        while i < n or hr:
            if not hr or (i < n and buildings[i][0] <= hr[0][1]):
                ...
                while hr and hr[0][1] <= x:
                    heappop(hr)
            height = -hr[0][0] if len(hr) > 0 else 0
